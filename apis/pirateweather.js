...
const PirateWeather = require('pirate-weather'),
...
class PirateWeatherAPI
{
	...
	{
		this.attribution = 'Powered by Pirate Weather';
		...
		this.pirateweather = new PirateWeather(apiKey);
		this.pirateweather.options({
		...
		this.pirateweatherTimeMachine = new PirateWeather(apiKey);
		this.pirateweatherTimeMachine.options({
			latitude: locationGeo[0],
			longitude: locationGeo[1],
			language: language,
			units: 'si',
	...
	update(forecastDays, callback)
	{
		...
		this.updateCache(this.pirateweatherTimeMachine, forecastDays, function ()
		{
			that.log.debug("Updating weather with Pirate Weather");
			that.pirateweather.get()
			...
			{
				that.log.error("Error retrieving weather report and forecast for Pirate Weather");
	...
	getConditionCategory(name, detail = false)
	{
		...
		{
			this.log.warn("Unknown Pirate Weather weather category " + name);
			return 0;
		}
	};
}
...
module.exports = {
	PirateWeatherAPI: PirateWeatherAPI
};
